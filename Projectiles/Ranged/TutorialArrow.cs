...
namespace TutorialMod.Projectiles.Ranged
{
    public class TutorialArrow : ModProjectile
    {
        ...
        public override void SetDefaults()
        {
            ...
            //アイテムのものとほぼ同じ。発射体のクリティカル率は持っている属性のクリティカル率を参照する。
            ...
            //extraUpdatesで追加された処理の回数分だけtimeleftも減少する
            
            Projectile.alpha = 255;//projectileのアルファ値...だがこれが初見だと厄介である。**255に近いほど** 透明になる。ややこしいね RGBAのAとは真逆
            Projectile.scale = 1.0f;//発射体の大きさの倍率。これを変えたところで見かけのサイズが変わるだけでヒットボックスには影響しない。ヒットボックスのサイズも変えたい場合はこのスケールを基に縦横幅を調整してやる必要がある

            //projectile.rotation = 0;//発射体の回転(radian) 3.141592...(Math.PIやMathHelper.Piを使うと3.14とか入力しなくていいので楽)で半回転。三角関数の基礎ってやつだよ
        ...
        public override void Kill(int timeLeft)//projectileが非active化されずにkillされた場合(貫通しきった、timeleftが0になった、Killが実行された場合)に呼び出される
        {
            ...
            if (!Projectile.noDropItem && Main.rand.NextBool(2))//アイテムを落とすように設定されている場合、1/2の確率でベースの矢をアイテムとして発生させる あってもなくてもいい
            {
                //Main.rand.NextBool(2)は、(カッコ内の数字)分の1の確率でtrueが返される
                //この場合は2なので二分の一、50%の確率ですね
